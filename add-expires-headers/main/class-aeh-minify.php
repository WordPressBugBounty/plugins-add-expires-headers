...
class AEH_Minify
{
	...
	public function __construct()
	{
		...
		/** @var AEH_Settings $this->settings */
	...
	public function aeh_minification()
	{
		...
		if (!$this->aeh_ignore_operations()) {
			...
			if (isset($this->minify_settings['process_css'])) {
				...
				add_action('wp_print_footer_scripts', array($this, 'aeh_merge_min_css_footer'), 9);
			}
	...
	public function aeh_min_async_css($tag, $handle, $src)
	{
		...
		if (stripos($tag, 'defer') === false && stripos($tag, 'async') === false) {
			...
			array_filter($this->aeh_ignore, function ($var) {
				return (stripos($var, '/cache/') === false);
			});
			if (isset($this->minify_settings['async_css'])) {
		...
		if (count($aeh_google_fonts) > 0) {
			...
			foreach ($aeh_google_fonts as $google_font_url) {
				...
				if ($this->minify_settings['inline_gfonts'] == true) {
					...
					$hash_code = hash('sha256', strtolower(trim($google_font_url)));
					wp_enqueue_style($string . '-aeh-fonts-' . $hash_code, $google_font_url, array(), null, 'all');
				}
			}
		}
	...
	public function aeh_is_local_domain($src)
	{
		...
		foreach ($locations as $l) {
			...
			if (stripos($src, $l) !== false && $result === false) {
				$result = true;
			}
		}
		return $result;
	...
	public function aeh_get_handle_url($src)
	{
		...
		if (!empty($this->site_home)) {
			...
			if (substr($aeh_handle_url, 0, strlen($alt_wp_content)) === $alt_wp_content) {
				$proceed = 1;
			}
		}
	...
	public function aeh_cachepath()
	{
		...
		// Base cache folder inside uploads
		$cache_root_dir = rtrim($upload_dir['basedir'], $this->aeh_slash()) . $this->aeh_slash() . 'aeh-cache';
		$cache_root_url = rtrim($upload_dir['baseurl'], '/') . '/aeh-cache';

		// Store root dir so we can delete it later if needed
		$this->cache_root_dir = $cache_root_dir;

		$aeh_cache_update_time = get_option('aeh-last-cache-update', '0');

		// e.g. /wp-content/uploads/aeh-cache/1733048260
		$cachebase    = $cache_root_dir . $this->aeh_slash() . $aeh_cache_update_time;
		$cachebaseurl = $cache_root_url . '/' . $aeh_cache_update_time;

		// e.g. .../1733048260/com and .../1733048260/ind
				...
				if (@mkdir($target, $dir_perms, true)) {
					if ($dir_perms != ($dir_perms & ~umask())) {
						...
						for ($i = 1, $c = count($folder_parts); $i <= $c; $i++) {
							@chmod(
								dirname($target) . $this->aeh_slash() . implode($this->aeh_slash(), array_slice($folder_parts, 0, $i)),
								$dir_perms
							);
						}
					}
				} else {
					// Fallback to WordPress helper
					wp_mkdir_p($target);
				}
			}
		}
	...
	public function aeh_purge_all_uninstall()
	{
		if (is_dir($this->cache_root_dir)) {
			$this->aeh_rrmdir($this->cache_root_dir);
		}
		return true;
	}
